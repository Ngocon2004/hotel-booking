...
- `app/(public)/`: public customer-facing routes such as `/`, `/about`, `/rooms`, `/search`, booking pages and my-bookings pages.
...
- `app/api/openapi/`: OpenAPI JSON route for Swagger UI, protected by `src/proxy.ts` so only admin can access it.
- `app/api-docs/`: Swagger UI HTML route, protected by `src/proxy.ts` so only admin can access it.
...
- Keep `/api-docs` and `/api/openapi` admin-only; do not link them from public pages.
